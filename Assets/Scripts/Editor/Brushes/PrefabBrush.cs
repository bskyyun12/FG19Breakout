...
public class PrefabBrush : GridBrushBase
{
    ...
    private static Transform GetObjectInCell(GridLayout gridLayout, Transform parent, Vector3Int postion)
    {
        int childCount = parent.childCount;
        Vector3 min = gridLayout.LocalToWorld(gridLayout.CellToLocalInterpolated(postion));
        Vector3 max = gridLayout.LocalToWorld(gridLayout.CellToLocalInterpolated(postion + Vector3Int.one));
        Bounds bounds = new Bounds((max + min) * 0.5f, max - min); // Bounds of a tile

        // Check child objects position against bounds of tile.
